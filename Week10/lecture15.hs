...
data Weird a = Fun (a->Bool) | Weird a
...
bindT (Leaf x) f = f x
bindT (Node t1 t2) f = Node (bindT t1 f) (bindT t2 f)
...
{- MORAL : IO operations only work with data "hidden" behind IO type wrapper;
        to get it out is only possible when you return another IO value-}
